...
const createIntern = async function (req, res) {
    try {
        ...
        // check there is any name in data if yes check should be valid and not reserved
        if (!requestbody.name) {
        ...
        if (checkMobile) {
            return res.status(400).send({ status: false, msg: "this mobile number is already reserved" })}
